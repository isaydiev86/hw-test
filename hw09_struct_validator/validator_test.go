...
type (
	User struct {
		...
		Role   UserRole        `validate:"in:admin,user,guest"`
...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:           TestString{StringSlice: []string{"regeerf", "43rf", "grte43"}},
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("subtest: %v", tt.name), func(t *testing.T) {
			...
			if tt.expectedErrs == nil {
				require.NoErrorf(t, errFromValidate, "no error: ", errFromValidate)
			}
